...
public class SplitTerrain : EditorWindow
{
    ...
    public void OnGUI()
    {
		...
		if (null != transform) {
			newHeightRes = transform.GetComponent<Terrain> ().terrainData.heightmapResolution;
			newDetailRes = transform.GetComponent<Terrain> ().terrainData.detailResolution;
			newSplatRes = transform.GetComponent<Terrain> ().terrainData.alphamapResolution;
		}
